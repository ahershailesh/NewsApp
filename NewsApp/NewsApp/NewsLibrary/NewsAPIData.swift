...
enum NewsAPIEndpoint: String {
    case topHeadLines = "top-headlines", everything = "everything", sources = "sources"
}
...
struct NewsAPIData: RequestDataProvider {
    var path: String {
        return "/\(endPoint.rawValue)?" + queryParam.map { "\($0.key)=\($0.value)" }.joined(separator: "&")
    }
    ...
    init(endPoint: NewsAPIEndpoint, query: String?, country: Country?, category: Category?, source: String?, apiKey: API_KEY = .NEWS_API) {
